...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ArchivedSection(props) {
			...
			/**
			 * Display title for one archived session id, matching the workspace
			 * list's own naming (the `sessions.byId` mirror carries the host
			 * `displayTitleOf(title, cwd, id)` projection: durable title, then
